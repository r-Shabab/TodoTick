...
const TaskLayout = ({ taskId, children }) => {
  const { deleteTodo, undoDelete, togglePinTodo, todos } = useContext(TaskContext);
      ...
      <div className="flex w-[5%] justify-center space-x-5 px-2">
        ...
        {todo.deleted ? (
          <button onClick={() => undoDelete(taskId)}>
            <RiArrowGoBackLine className="h-4 w-4 fill-color-text hover:fill-color-primary lg:h-7 lg:w-7 transition-all duration-0" />
          </button>
        ) : (
